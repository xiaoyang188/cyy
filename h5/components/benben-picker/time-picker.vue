...
export default {
  ...
  methods: {
    ...
    checkValue(value) {
      let strReg = this.second ? /^\d{2}:\d{2}:\d{2}$/ : /^\d{2}:\d{2}$/,
        example = this.second ? '18:00:05' : '18:00'
    ...
    getDval() {
      ...
      if (value && this.checkValue(value)) {
        ...
        dVal = this.second ? ['00', '00', '00'] : ['00', '00']
      }
      return dVal
    },
    initData() {
      ...
      console.log(' hours.indexOf(dVal[0])', hours.indexOf(dVal[0]))
      pickVal = this.second
        ? [
      ...
      this.range = dateData
      if (pickVal.join(',') != this.pickVal.join(',')) {
        this.handlerChange({ detail: { value: pickVal } })
        this.$nextTick(() => {
    ...
    handlerChange(e) {
      ...
      full = this.second ? `${hour + ':' + minute + ':' + second}` : `${hour + ':' + minute}`
      this.$emit('change', {
        result: result,
        value: full,
        obj: obj,
