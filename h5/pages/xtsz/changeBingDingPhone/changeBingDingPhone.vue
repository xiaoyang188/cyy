<template>
    <page-body   >
        <view class="page" >
    <view class="flex flex-direction flex-wrap align-stretch justify-center benben-position-layout flex changeBingDingPhone_flex_0_babdd"    :style="{height:(88+StatusBarRpx)+'rpx',paddingTop:StatusBarRpx+'rpx', }">
            <view   class='flex flex-wrap align-stretch justify-between changeBingDingPhone_fd0_0_babdd'      >
            <view   class='flex flex-wrap align-center changeBingDingPhone_fd0_0_c0_babdd'   @tap.stop="handleJumpDiy" data-type="back" data-url="1"   >
        <text class='fu-iconfont2  changeBingDingPhone_fd0_0_c0_c0_babdd'  >&#xe794;</text>
</view>
    <view   class='flex flex-wrap align-center'      >
        <text class='changeBingDingPhone_fd0_0_c1_c0_babdd'  >{{$t('修改绑定手机号')}}</text>
</view>
    <view   class='flex align-center justify-end changeBingDingPhone_fd0_0_c2_babdd'      >
        </view>
</view>
...
<view :style="{height: (88+StatusBarRpx)+'rpx'}"  ></view>
 ...
 <view class="flex flex-direction align-center benben-flex-layout changeBingDingPhone_flex_1_babdd"    >
        <text class='changeBingDingPhone_fd1_0_babdd'  >{{$t('我们要对您绑定的手机号进行验证：')}}</text>
<text class='changeBingDingPhone_fd1_1_babdd'  >{{dataMessage.mobile}}</text>
<benben-digital-password  :box-num='4' mode='number' class-text="flex flex align-center justify-center changeBingDingPhone_fd1_2_babdd"  v-model="code" :init-open.sync="xianshi">
<template #default="{ content, active, index }">
<view :key="index" v-if="active" class="flex align-center justify-center active-digital-password changeBingDingPhone_flex_password_active_fd1_2_babdd" >
  {{ content }}
</view>
<view :key="index" v-else class="flex align-center justify-center changeBingDingPhone_flex_password_active_fd1_2_babdd" >
  {{ content }}
</view>
</template>
</benben-digital-password>
    <view   class='flex flex-wrap align-center justify-center'      >
        <benben-send-verification-code key='1688627152855' ref="vCode1688627152855" class=' changeBingDingPhone_fd1_3_c0_babdd'  :after-text="$t('后重新获取')" :before-text="$t('获取验证码')" type='4' :is-environment-verification='true'  :phone="dataMessage._mobile" ></benben-send-verification-code>
</view>
<button class='changeBingDingPhone_fd1_4_babdd'  @tap.stop="nextPageFunc()"  >{{$t('下一步')}}</button>
</view>
...
*   invite_code【邀请码】 **/ "dataMessage":{"invite_code":"","address":"","address_code":"","hobby":"","tags":"","id":0,"userlevel_id":0,"mobile":"","real_name":"","avatar":"","account":"","email":"","birthday":"","gender":"","create_time":"","user_money":"","freeze_money":"","total_consumption_money":"","score":"","_mobile":"","coupon":0,"collection_count":0,"browse_count":0,"total_money":0,"certified":0,"nickname":"","is_pay_password":0,"follow_count":0},/** code【验证码】 **/ "code":"","xianshi":true};
  ...
  let data5f6db4db8abcf = await this.$api.post(global.apiUrls.post5f6db4db8abcf, {
    mobile:this.dataMessage._mobile,code:this.code,type:'4'
  });
...
<style lang="scss" scoped>
    ...
    .changeBingDingPhone_flex_password_active_fd1_2_babdd{
    ...
    .changeBingDingPhone_flex_0_babdd{
