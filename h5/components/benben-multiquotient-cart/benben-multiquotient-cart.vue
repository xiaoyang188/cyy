<template>
  <view>
    <benben-input-popup ref="inputPopup" z-index="1000" :min="1" :max="activeInfo.max" :step="1" @change="setnumber" />
    <!-- 未登录状态  显示空购物车 按钮为去登录 -->
    ...
    <template v-if="isLogin">
      <benben-empty :list-data="cartListArr">
      ...
      <!-- 购物车列表 -->
      <view class="cart-box" :class="type">
...
export default {
  ...
  methods: {
    addingNumChange() {
      if (this.addingNum !== 1) {
        this.$emit('update:adding-num', false)
      }
    },
    openInput(value, index, i, max) {
      this.activeInfo.index = index
      this.activeInfo.i = i
      this.activeInfo.max = max
      this.$refs.inputPopup.open(value)
    },
    setnumber(e) {
      this.numberChange(e, this.activeInfo.index, this.activeInfo.i)
    },
    openPopup(item) {
    ...
    delCart(type) {
      ...
      if (ids.length == 0) return this.$message.info(type == 1 ? t('请选择要删除的商品') : t('请选择要移动的商品'))

      this.$util.showModal({
        title: t('提示'),
        content: type == 1 ? t('确认删除吗?') : t('确认移入收藏夹吗?'),
        success: (res) => {
          if (res.confirm) {
            if (type == 1) {
...
.cart-footer {
  ...
  left: 0;
  position: fixed;
  bottom: var(--benben-window-bottom, 0px);
  width: 100%;
  z-index: 98;
  background: #ffffff;
  display: flex;
