<template>
  <page-body>
    <view class="page">
      <view class="flex flex-wrap align-center justify-between benben-position-layout flex order_flex_0_babdd"
        ...
        <text class="order_fd0_1_babdd">{{ $t('我的订单') }}</text>
        <view class="flex flex-wrap align-center order_fd0_0_babdd"></view>
      </view>
      <view :style="{ height: 88 + StatusBarRpx + 'rpx' }"></view>
      <!---多商订单tabsflex布局开始-->
      <view class="flex align-stretch benben-flex-layout">
        <view>
          <benben-flex-tabs class-text="order_benbenTabsfd1_0_babdd" v-model="type" ref="benben_tabsfd1_0"
              ...
              <view class="benben-tabs-content" id="benben_tabsfd1_0-content">
                <view id="benben_tabsfd1_0-title" class="benben-tabs-title flex align-center flex">
                  ...
                  <view
                    ...
                    @tap="type = 'finish'" :id="`benben_tabsfd1_0-title-item-${'finish'}`">
                    <text>{{ $t('待评价') }}</text>
                  </view>
                  <view
                    :class="{ checkTitlefd1_0_babdd: type == '6', 'flex flex-wrap align-center order_titlefd1_0_c8_babdd': true }"
                    @tap="type = '6'" :id="`benben_tabsfd1_0-title-item-${'6'}`">
                    <text>{{ $t('退款/售后') }}</text>
                  </view>
                </view>
                <view :style="{ left: tabsInfofd1_0.lineleft, maxWidth: tabsInfofd1_0.lineWidth }"
                  id="benben_tabsfd1_0-line" class="benben-tabs-line flex benben-flex-tabs-line order_linefd1_0_babdd"
      ...
      <!---多商订单tabsflex布局结束-->
      <!---多商订单商品flex布局开始-->
      <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout order_flex_2_babdd"
        v-if="type != '6'">
        <template v-for="(item, key0) in dataList">
          <view class="flex flex-direction flex-wrap align-stretch order_fd2_0_babdd" @tap.stop="handleJumpDiy"
            ...
            <view class="flex flex-wrap align-center justify-between order_fd2_0_c0_babdd">
              <view class="flex flex-wrap align-center">
                <text class="order_fd2_0_c0_c0_c0_babdd">{{ `订单号：${item.order_sn}` }}</text>
              </view>
              <text class="order_fd2_0_c0_c1_babdd" v-if="item.status == '0'">{{ $t('待付款') }}</text>
              ...
              <text class="order_fd2_0_c0_c1_1_babdd" v-if="in_array(item.status, '6,-1,-2,-3')">{{ $t('交易关闭') }}</text>
            ...
            <template v-for="(child, key1) in item.goods">
              <view class="flex align-stretch order_fd2_0_c1_babdd" :key="key1">
                ...
                <view class="flex flex-direction flex-wrap align-stretch flex-sub order_fd2_0_c1_c1_babdd">
                  ...
                  <text class="order_fd2_0_c1_c1_c1_babdd">{{ child.sku_name }}</text>
                  <view class="flex flex-wrap align-center justify-between">
                    ...
                    <view class="flex flex-wrap align-center">
                      <text class="order_fd2_0_c1_c1_c2_c1_c0_babdd">x</text>
                      <text class="order_fd2_0_c1_c1_c2_c1_c0_babdd">{{ child.num }}</text>
                    </view>
                  </view>
                </view>
              </view>
            ...
            <view class="flex flex-wrap align-center justify-end order_fd2_0_c3_babdd">
              ...
              <button class="order_fd2_0_c3_c0_babdd" @tap.stop="getinvoice_order_idFunc(item.aid)"
                v-if="item.btn_list.apply_invoice == '1'">
                {{ $t('申请开票') }}
              </button>
              <button class="order_fd2_0_c3_c0_babdd" @tap.stop="handleJumpDiy" data-type="navigateTo"
                :data-url="`/pages/fp/invoiceDetail/invoiceDetail?order_id=${item.aid}`"
                v-if="item.btn_list.view_invoice == '1'">
                {{ $t('查看发票') }}
              </button>
              <button class="order_fd2_0_c3_c1_babdd"
                @tap.stop="isMultiplePackagesFunc(item.is_multiple_package, item.aid)"
                v-if="item.btn_list.view_logistics == '1'">
                {{ $t('查看物流') }}
              </button>
              <button class="order_fd2_0_c3_c1_babdd" @tap.stop="getDeleteIDFunc(item.aid)"
                v-if="item.btn_list.delete_order == '1'">
                {{ $t('删除订单') }}
              </button>
              <button class="order_fd2_0_c3_c2_babdd" @tap.stop="getCancelIDFunc(item.aid)"
                v-if="item.btn_list.cancel_order == '1'">
                {{ $t('取消订单') }}
              </button>
              ...
              <button class="order_fd2_0_c3_c3_1_babdd" @tap.stop="remindShipFunc(item.aid)"
                v-if="item.btn_list.remind_send == '1'">
                {{ $t('提醒发货') }}
              </button>
              <button class="order_fd2_0_c3_c3_1_babdd" @tap.stop="gettakeoverIDFunc(item.aid)"
                v-if="item.btn_list.confirm_receive == '1'">
                {{ $t('确认收货') }}
              </button>
              <button class="order_fd2_0_c3_c3_3_babdd" @tap.stop="handleJumpDiy" data-type="navigateTo"
                :data-url="`/pages/ddgl/evaluation/evaluation?id=${item.aid}`" v-if="item.btn_list.evaluate == '1'">
                {{ $t('评价') }}
              </button>
              <button class="order_fd2_0_c3_c3_1_babdd" @tap.stop="handleJumpDiy" data-type="navigateTo"
              ...
              <button class="order_fd2_0_c3_c3_3_babdd"
                v-if="item.status == '0' && item.order_type == 'offline_pay'">{{ $t('转账详情') }}</button>
              <button class="order_fd2_0_c3_c3_3_babdd"
      ...
      <benben-popup v-model="popupShow1679903057541" :mask="true" :mask-close-able="true" mode="bottom">
        ...
        <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout order_flex_4_babdd">
          <view class="flex flex-direction flex-wrap align-stretch order_fd4_0_babdd">
            ...
            <scroll-view class="order_fd4_0_c2_babdd" :scroll-x="false" :scroll-y="true" :show-scrollbar="false"
              :enhanced="true">
              <view class="flex flex-direction align-stretch benben-scroll flex">
                <benben-select-diy ref="showSelectPopup1693558120412" class-text="flex-direction flex"
                  ...
                  <benben-select-item v-for="(item, key0) in cancelType" :hand-value="item.aid" :key="key0">
                    <template #selected>
                      <view class="flex justify-between flex order_fd4_0_c2_c0_c0_babdd">
                        ...
                        <image class="order_fd4_0_c2_c0_c0_c1_babdd" mode="aspectFit" :src="STATIC_URL + '448.png'">
                        </image>
                      </view>
                    </template>
                    <template #no-selected>
                      <view class="flex justify-between flex order_fd4_0_c2_c0_c0_babdd">
                        <text class="order_fd4_0_c2_c0_c0_c0_babdd">{{ item.reason }}</text>
                        <image class="order_fd4_0_c2_c0_c0_c1_babdd" mode="aspectFit" :src="STATIC_URL + '336.png'">
                        </image>
                      </view>
                    </template>
                  </benben-select-item>
      ...
      <view class="flex flex-direction align-stretch benben-flex-layout order_flex_5_babdd" v-if="type == '6'">
        <template v-for="(item, key0) in dataAfterList">
          <view class="flex flex-direction flex-wrap align-stretch order_fd5_0_babdd" @tap.stop="handleJumpDiy"
            data-type="navigateTo" :data-url="`/pages/ddgl/detailsRefund/detailsRefund?id=${item.aid}`" :key="key0">
            <view class="flex flex-wrap align-center order_fd5_0_c0_babdd">
              ...
              <view class="flex flex-wrap align-center" v-if="item.refund_type == '1' || item.refund_type == '2'">
                ...
                <text class="order_fd5_0_c0_c2_c0_1_babdd"
                  v-if="item.status == '1' && item.express_no == ''">{{ $t('待寄回') }}</text>
                ...
                <text class="order_fd5_0_c0_c2_c0_1_babdd"
                  v-if="item.status == '1' && item.express_no != ''">{{ $t('待退款') }}</text>
              </view>
              <view class="flex flex-wrap align-center" v-if="item.refund_type == '3'">
                ...
                <text class="order_fd5_0_c0_c2_c0_1_babdd"
                  v-if="item.status == '1' && item.express_no == ''">{{ $t('待寄回') }}</text>
                <text class="order_fd5_0_c0_c2_c0_1_babdd"
                  v-if="item.status == '1' && item.express_no != ''">{{ $t('待收货') }}</text>
            ...
            <view class="flex flex-wrap align-center justify-end order_fd5_0_c2_babdd">
              ...
              <button class="order_fd5_0_c2_c0_babdd" @tap.stop="getRefundDeleteIDFunc(item.aid)"
              ...
              <button class="order_fd5_0_c2_c1_babdd" @tap.stop="handleJumpDiy" data-type="navigateTo"
      ...
      <benben-popup v-model="popupShow1679650134928" :mask="true" :mask-close-able="true" mode="bottom">
        ...
        <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout order_flex_6_babdd">
          <view class="flex flex-wrap align-center justify-center order_fd6_0_babdd">
            ...
            <image class="order_fd6_0_c3_babdd" mode="aspectFit" :src="STATIC_URL + '124.png'"
              @tap.stop="popupShow1679650134928 = false"></image>
          </view>

          <scroll-view class="order_fd6_1_babdd" :scroll-x="false" :scroll-y="true" :show-scrollbar="false"
            :enhanced="true">
            <view class="flex flex-direction align-stretch benben-scroll flex">
              <benben-select-diy ref="showSelectPopup1687657939559" class-text="flex-direction align-stretch flex"
                :items.sync="invoiceData" v-model="invoice_id" default-type="aid" default-label="username"
                :allow-cancel="true" type="radio" :disabled="false">
                    ...
                    <view class="flex align-center flex order_fd6_1_c0_c0_babdd">
                      ...
                      <view class="flex flex-direction flex-wrap align-stretch flex-sub order_fd6_1_c0_c0_c1_babdd">
                        <view class="flex flex-wrap align-center">
                          ...
                          <view class="flex flex-wrap align-center order_fd6_1_c0_c0_c1_c0_c1_babdd"
                          ...
                          <view class="flex flex-wrap align-center order_fd6_1_c0_c0_c1_c0_c2_babdd">
                            <text class="order_fd6_1_c0_c0_c1_c0_c1_c0_babdd"
                              v-if="item.type == '1'">{{ $t('个人') }}</text>
                            <text class="order_fd6_1_c0_c0_c1_c0_c2_c0_1_babdd"
                              v-if="item.type != '1'">{{ $t('企业') }}</text>
                          </view>
                        </view>
                        <view class="flex flex-wrap align-center order_fd6_1_c0_c0_c1_c1_babdd">
                          ...
                          <view class="flex flex-wrap align-center" @tap.stop="handleJumpDiy" data-type="navigateTo"
                            ...
                            <image class="order_fd6_1_c0_c0_c1_c1_c1_c0_babdd" mode="aspectFit"
          ...
          <button class="order_fd6_2_babdd" @tap.stop="handleJumpDiy" data-type="navigateTo"
            :data-url="`/pages/fp/applyInvoic/applyInvoic`" v-if="invoice_id == ''">
            {{ $t('添加新的抬头') }}
          </button>
