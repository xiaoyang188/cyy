<template>
  <page-body>
    <view class="page">
      <view class="flex align-stretch benben-position-layout flex commodityList_flex_0_babdd"
        ...
        <view class="flex align-center justify-between flex-sub commodityList_fd0_0_babdd">
          ...
          <view class="flex align-center flex-sub commodityList_fd0_0_c1_babdd">
            ...
            <benben-input class="flex-sub commodityList_fd0_0_c1_c1_babdd" type="text" :placeholder="$t('请输入商品名称')"
              confirm-type="done" :maxlength="-1" placeholder-style="color:var(--benbenFontColor1);font-size:28rpx"
              @confirm="topSearchFunc()" v-model="keyword" v-if="showId == '1'" />
            <benben-input class="flex-sub commodityList_fd0_0_c1_c1_babdd" type="text" :placeholder="$t('请输入店铺名称')"
              confirm-type="done" :maxlength="-1" placeholder-style="color:var(--benbenFontColor1);font-size:28rpx"
              @confirm="topSearchFunc()" v-model="keyword" v-if="showId == '2'" />
            <image class="commodityList_fd0_0_c1_c3_babdd" mode="aspectFit" :src="STATIC_URL + '1485.png'"
              @tap.stop="cleanKeywordFunc()" v-if="keyword != ''"></image>
          </view>
          <view class="flex flex-wrap align-center commodityList_fd0_0_c2_babdd"
            v-if="appSystemIdentification == 'Wechat'"></view>
        </view>
      </view>
      <view :style="{ height: 88 + StatusBarRpx + 'rpx' }"></view>
      <view class="flex benben-position-layout flex align-center commodityList_flex_1_babdd"
        ...
        <view class="flex self-center align-center flex-sub commodityList_fd1_0_babdd" v-if="showId == '1'">
          ...
          <view class="flex align-center commodityList_fd1_0_c1_babdd" @tap.stop="switchShopListFunc(2)">
            ...
            <image class="commodityList_fd1_0_c1_c1_babdd" mode="aspectFit" :src="STATIC_URL + '396.png'"
              v-if="sort_field != '2'"></image>
            <image class="commodityList_fd1_0_c1_c1_babdd" mode="aspectFit" :src="STATIC_URL + '1521.png'"
              v-if="sort_field == '2' && sort_type == 'desc'"></image>
            <image class="commodityList_fd1_0_c1_c1_babdd" mode="aspectFit" :src="STATIC_URL + '65.png'"
          ...
          <view class="flex align-center commodityList_fd1_0_c3_babdd" v-if="showId == '1'">
            <image class="commodityList_fd1_0_c3_c0_babdd" mode="aspectFit" :src="STATIC_URL + '50.png'"
              @tap.stop="switchcutFunc(2)" v-if="cutId == '1'"></image>
            <image class="commodityList_fd1_0_c3_c0_babdd" mode="aspectFit" :src="STATIC_URL + '51.png'"
              @tap.stop="switchcutFunc(1)" v-if="cutId == '2'"></image>
          </view>
          <view class="flex flex-wrap align-center commodityList_fd1_0_c4_babdd"
            @tap.stop="popupShow1679566295078 = true" v-if="showId == '1'">
      ...
      <benben-popup v-model="popupShow1679566295078" :mask="true" :mask-close-able="true" mode="right">
        ...
        <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout commodityList_flex_2_babdd">
          <view class="flex flex-wrap align-center justify-between commodityList_fd2_0_babdd"
            :style="{ 'margin-top': `2vh` }">
            <text class="commodityList_fd2_0_c0_babdd">{{ $t('筛选') }}</text>
          </view>
          <view class="flex flex-direction flex-wrap align-stretch flex-sub">
            <view class="flex flex-wrap align-center justify-between commodityList_fd2_1_c0_babdd"
              @tap.stop="showBrandFunc()">
              <text class="commodityList_fd2_1_c0_c0_babdd">{{ $t('品牌') }}</text>
              <image class="commodityList_fd2_1_c0_c1_babdd" mode="aspectFit" :src="STATIC_URL + '291.png'"
                v-if="is_showbrand === false"></image>
              <image class="commodityList_fd2_1_c0_c1_babdd" mode="aspectFit" :src="STATIC_URL + '1515.png'"
                v-if="is_showbrand === true"></image>
            </view>
            <benben-select-diy ref="showSelectPopup1679567052685"
              class-text="flex-wrap flex commodityList_fd2_1_c1_babdd" :items.sync="dataDrandList" v-model="brand_id"
              default-type="aid" default-label="name" :allow-cancel="true" type="radio" :disabled="false"
              v-if="is_showbrand === true">
              <benben-select-item v-for="(item, key0) in dataDrandList" :hand-value="item.aid" :key="key0">
                <template #selected>
                  <view class="flex align-center self-center justify-center flex commodityList_fd2_1_c1_c0_babdd"
                    :style="{ 'min-width': `160rpx` }">
                    <text class="commodityList_fd2_1_c1_c0_c0_babdd">{{ item.name }}</text>
                  </view>
                </template>
                <template #no-selected>
                  <view class="flex align-center justify-center flex commodityList_fd2_1_c1_c1_babdd"
                    :style="{ 'min-width': `160rpx` }">
          ...
          <view class="flex flex-direction flex-wrap align-stretch flex-sub commodityList_fd2_2_babdd"
            v-if="is_showPrice === true">
            <view class="flex flex-wrap align-center justify-between commodityList_fd2_1_c0_babdd"
              @tap.stop="showPrcieFunc()">
            ...
            <benben-price-range class="commodityList_fd2_2_c1_babdd" :min-price.sync="min_price"
              :max-price.sync="max_price" active-color="var(--benbenFontColor4)" price-color="var(--benbenbgColor0)"
              name-key="name" max-key="max" min-key="min"></benben-price-range>
          </view>
          <view class="flex flex-direction flex-wrap align-stretch flex-sub">
      ...
      <!---商品列表_商品flex布局开始-->
      <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout commodityList_flex_3_babdd"
        v-if="cutId == '1' && showId == '1'">
        <template v-for="(item, key0) in goods_List">
          <view class="flex align-stretch commodityList_fd3_0_babdd commodityList_goods-card" @tap.stop="handleJumpDiy"
            ...
            <view class="flex flex-direction flex-wrap align-stretch flex-sub commodityList_fd3_0_c1_babdd">
              ...
              <view class="flex flex-wrap align-center commodityList_fd3_0_c1_tags_babdd"
                v-if="item.active_info && item.active_info.active_id">
                <text class="commodityList_fd3_0_c1_c1_c0_c0_babdd"
                  v-if="item.active_info.type == '1'">{{ $t('秒杀') }}</text>
                <text class="commodityList_fd3_0_c1_c1_c0_c0_1_babdd"
                  v-if="item.active_info.type == '2'">{{ $t('拼团') }}</text>
                <text class="commodityList_fd3_0_c1_c1_c0_c0_1_babdd"
                  v-if="item.active_info.type == '3'">{{ $t('预售') }}</text>
                <text class="commodityList_fd3_0_c1_c1_c0_c0_1_babdd"
                  v-if="item.active_info.type == '4'">{{ $t('特价') }}</text>
              </view>
              ...
              <view class="flex flex-wrap align-center commodityList_fd3_0_c1_c3_babdd"
                v-if="item.coupon && item.coupon.name">
      ...
      <!---商品列表_商品flex布局结束-->
      <benben-specs-popup ref="specsPopup1719633630974" class="commodityList_flex_specs_4_babdd" price-name="shop_price"
        :speclist="dataDetail.spec_list" :skulist="dataDetail.sku_list" :sku-id.sync="sku_id" :sku-name.sync="sku_name"
        :price.sync="dataDetail.shop_price" :thumb.sync="dataDetail.thumb" :stock.sync="dataDetail.stock"
        :number.sync="buy_num" @addCart="addCarFunc()"></benben-specs-popup>
      <!---商品列表_商品1flex布局开始-->
      <view class="flex flex-wrap align-stretch benben-flex-layout commodityList_flex_5_babdd"
        v-if="cutId == '2' && showId == '1'">
        <view class="flex-sub">
          <benben-waterfalls ref="waterfalls1706929510943" :waterfall-data.sync="waterfallsfd5_0" v-model="goods_List"
            id-key="aid" add-time="200" column-count="2" left-gap="10" right-gap="10" column-gap="10">
            <benben-waterfalls-item :sort-index="sortIndex" :key="sortIndex"
              v-for="(waterfall, sortIndex) in waterfallsfd5_0">
              <template v-for="waterfallIndex in waterfall">
                <template v-if="goods_List[waterfallIndex]">
                  <view class="flex flex-direction align-stretch commodityList_fd5_0_c0_babdd commodityList_goods-card"
                    ...
                    <view class="commodityList_fd5_0_c0_imgwrap_babdd">
                      <image class="commodityList_fd5_0_c0_c0_babdd" mode="aspectFill"
                        :src="goods_List[waterfallIndex].thumb"></image>
                    </view>
                    <view class="commodityList_fd5_0_c0_body_babdd">
                      <view class="commodityList_fd5_0_c0_c1_babdd">
                        <text class="commodityList_fd5_0_c0_c1_c0_babdd"
                          v-if="goods_List[waterfallIndex].is_new == '1'">{{ $t('新品') }}</text>
                        <text class="commodityList_fd5_0_c0_c1_c1_babdd">{{ goods_List[waterfallIndex].name }}</text>
                      </view>
                      <view class="flex flex-wrap align-center commodityList_fd5_0_c0_tags_babdd"
                        ...
                        <text class="commodityList_fd5_0_c0_c2_c0_c0_babdd"
                          v-if="goods_List[waterfallIndex].active_info.type == '1'">
                          {{ $t('秒杀') }}
                        </text>
                        <text class="commodityList_fd5_0_c0_c2_c0_c0_1_babdd"
                          v-if="goods_List[waterfallIndex].active_info.type == '2'">
                          {{ $t('拼团') }}
                        </text>
                        <text class="commodityList_fd5_0_c0_c2_c0_c0_1_babdd"
                          v-if="goods_List[waterfallIndex].active_info.type == '3'">
                          {{ $t('预售') }}
                        </text>
                        <text class="commodityList_fd5_0_c0_c2_c0_c0_1_babdd"
                          v-if="goods_List[waterfallIndex].active_info.type == '4'">
                          {{ $t('特价') }}
                        </text>
                      </view>
                      <view class="flex flex-wrap align-baseline commodityList_fd5_0_c0_c2_babdd">
                        ...
                        <text class="commodityList_fd5_0_c0_c2_c2_babdd">
                          <text
                            class="commodityList_price1_fd5_0_c0_c2_c2_babdd">{{ goods_List[waterfallIndex].shop_price | frontPrice }}</text>
                          <text
                            class="commodityList_price2_fd5_0_c0_c2_c2_babdd">{{ goods_List[waterfallIndex].shop_price | laterPrice }}</text>
                        </text>
                      </view>
                      <view class="flex flex-wrap align-center justify-between commodityList_fd5_0_c0_c3_babdd">
                        <view class="flex flex-wrap align-center commodityList_fd5_0_c0_c3_market_babdd"
                          v-if="goods_List[waterfallIndex].market_price">
                          <text class="commodityList_fd5_0_c0_c3_c0_c0_babdd">{{ $t('￥') }}</text>
                          <text
                            class="commodityList_fd5_0_c0_c3_c0_c0_babdd">{{ goods_List[waterfallIndex].market_price }}</text>
                        </view>
                        <view class="flex flex-wrap align-center commodityList_fd5_0_c0_c3_sales_babdd">
                          ...
                          <text
                            class="commodityList_fd5_0_c0_c2_c4_babdd">{{ goods_List[waterfallIndex].sales_sum }}</text>
                        </view>
                      </view>
                      <view class="flex flex-wrap align-center commodityList_fd5_0_c0_c5_babdd"
                        ...
                        <view class="flex flex-wrap align-center commodityList_fd5_0_c0_c4_c0_babdd">
                          <text
                            class="commodityList_fd5_0_c0_c4_c0_c0_babdd">{{ goods_List[waterfallIndex].coupon.name }}</text>
                        </view>
                      </view>
                    </view>
                  </view>
