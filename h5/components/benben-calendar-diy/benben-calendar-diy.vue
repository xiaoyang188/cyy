<template>
  <view class="fu-calendar-wrap" :style="defaultCssVar">
    ...
    <view class="fu-calendar-data fu-flex fu-justify-center fu-align-center fu-margin fu-height-72" v-if="changeTitle">
      <view v-if="changeYear" class="fu-arrow fu-flex fu-justify-between fu-align-center" @click="prevYear">
        <text class="fu-iconfont2" :style="[yaColor]">&#xE807;</text>
      </view>
      <view v-if="changeMonth" class="fu-arrow single fu-flex fu-justify-between fu-align-center" @click="prevMonth">
      ...
      <view v-if="changeMonth" class="fu-arrow single fu-flex fu-justify-between fu-align-center" @click="nextMonth">
        <text class="fu-iconfont2" :style="[maColor]">&#xE7f2;</text>
      </view>
      <view v-if="changeYear" class="fu-arrow fu-flex fu-justify-between fu-align-center" @click="nextYear">
        <text class="fu-iconfont2" :style="[yaColor]">&#xE7af;</text>
      </view>
    </view>
    <!-- 日历选择 end -->
      ...
      >
        <view
          ...
          :class="{
            ...
            'checked-style': mode === 'multiple' && item.ischecked,
          }"
        >
          {{ item.day }}
          <view class="fu-calendar-text" v-show="start === item.timestamp && mode == 'range' && end !== start">{{ startText }}</view>
          <view class="fu-calendar-text" v-show="end === item.timestamp && mode == 'range' && end !== start">{{ endText }}</view>
          <view class="fu-calendar-text" v-show="end === start && mode == 'range'">{{ $t('当天') }}</view>
        </view>
      </view>
    </view>
    <!-- 日历 end -->
...
export default {
  ...
  props: {
    mode: {
      type: String,
      default: 'range', //日期模式 range date signin multiple
    },
    yearArrowColor: {
      type: String,
    ...
    noPast: {
      type: Boolean,
      default: false, //是否禁用过去日期是否可选择
    },
    noCurrent: {
      type: Boolean,
      default: false, //是否禁用当前日期
    },
    changeTitle: {
      type: Boolean,
  ...
  computed: {
    ...
    calendar() {
      ...
      for (let index = -startWeek; index < totalDay; index++) {
        ...
        let isFuture = false
        if (!this.noCurrent && this.currentTimestamp == timestamp) {
          isFuture = true
        } else if (!this.noPast && timestamp < this.currentTimestamp) {
          isFuture = true
        } else if (this.future && timestamp > this.currentTimestamp) {
          isFuture = true
        }
        let ischecked = false
        if (this.mode == 'multiple') {
          for (var i = 0; i < this.checkedDaysArr.length; i++) {
            if (dayjs(timestamp).isSame(dayjs(this.checkedDaysArr[i]))) {
              ischecked = true
              break
            }
          }
  ...
  methods: {
    updatedTimeRange() {
      ...
      this.end = this.endTime ? dayjs(this.endTime).valueOf() : ''
      this.start = this.startTime ? dayjs(this.startTime).valueOf() : ''
    },
    calendarCalss(item) {
      return {
    ...
    changeCalendar(item) {
      ...
      // 未来日期 上下月日期不可选择
      if (!item.isFuture || (item.isNot && !this.extendMonth)) return
      ...
      if (this.mode == 'range') {
        ...
        if (!this.start && !this.end) {
          ...
          } else if (this.start > item.timestamp) {
            this.end = this.start
            this.start = item.timestamp
          }
        } else {
      ...
      if (this.mode === 'multiple') {
        // 检查当前点击的日期是否已经存在于数组中
        const index = this.checkedDaysArr.findIndex((day) => dayjs(day).isSame(dayjs(item.timestamp)))
        if (index === -1) {
          // 如果不存在，则添加到数组中
          this.checkedDaysArr.push(dayjs(item.timestamp).format('YYYY-MM-DD'))
        } else {
          this.checkedDaysArr.splice(index, 1)
        }
        item.ischecked = !item.ischecked
        this.$forceUpdate()

        let arr = []
        this.checkedDaysArr.forEach((item) => {
          arr.push(item)
        })
        this.$emit('change', arr)
        this.$emit('update:signin-data', arr)
      }
    },
  },
}
...
.range-active {
  color: var(--range-text-color, #333);
...
.fu-calendar {
  .fu-calendar-item {
    ...
    .checked-style {
      background: var(--checked-bg-color, #eb681e);
