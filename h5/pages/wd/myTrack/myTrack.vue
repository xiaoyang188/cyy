<template>
    <page-body  :loading='benbenPageloading'  >
        <view class="page" >
    <view class="flex align-stretch benben-position-layout flex myTrack_flex_0_babdd"    :style="{height:(88+StatusBarRpx)+'rpx',paddingTop:StatusBarRpx+'rpx', }">
    ...
    <view   class='flex align-center justify-end myTrack_fd0_0_c0_babdd' :style="{'margin-right':`${ appSystemIdentification == 'Wechat' ?  '30%':''}`}"     >
            <view   class='flex flex-wrap align-center'   @tap.stop="switchTypeFunc()" v-if= " appSystemIdentification!='Wechat'"  >
        <text class='myTrack_fd0_0_c2_c0_c0_babdd'    v-if= " dataList.length!=0 && is_edit===false"  >{{$t('编辑')}}</text><text class='myTrack_fd0_0_c2_c0_c0_babdd'    v-if= " is_edit===true && dataList.length!=0"  >{{$t('完成')}}</text>
</view>
</view>
</view>
...
<benben-popup v-model="popupShow1691652180194" :mask="true" :mask-close-able="true"  mode='center'  >
 ...
 <view class="flex benben-flex-layout flex-wrap align-center"    >
        <benben-calendar-diy  mode='range' year-arrow-color='#666666' month-arrow-color='#666666' start-text=' ' end-text=' ' active-bg-color='rgba(45, 175, 65, 1)' range-bg-color='rgba(45, 175, 65, 0.5)' :future='false' :change-title='true' :change-year='true' :change-month='true' class='myTrack_fd1_0_babdd'  @change="queryListFunc()"  :start-time.sync="start_time" :end-time.sync="end_time"></benben-calendar-diy>
</view>
...
<image class='myTrack_fd2_0_c4_babdd'  mode="aspectFit"  :src='STATIC_URL+"85.png"' @tap.stop="dltDateFunc()" v-if= " start_time!=''"></image>
</view>
    <view   class='flex align-center myTrack_fd2_1_babdd'   @tap.stop="switchTypeFunc()" v-if= " appSystemIdentification=='Wechat'"  >
        <text class='myTrack_fd2_1_c0_babdd'    v-if= " dataList.length!=0 && is_edit===false"  >{{$t('编辑')}}</text><text class='myTrack_fd2_1_c0_babdd'    v-if= " is_edit===true && dataList.length!=0"  >{{$t('完成')}}</text>
</view>
</view>
...
<!---flex布局flex布局结束-->
        <benben-goods-footprint time-format='MM月DD日' price-before='￥'  class='myTrack_flex_footprint_4_babdd' :is-editor.sync='is_edit' :goods-list.sync='dataList' :goods-id.sync='shop_id' v-model='detele_id' @handleDel="openPopFunc()" @handleGoods="jumpFunc()"  >
...
<!---订单列表flex布局开始-->
 <view class="flex flex-wrap align-center justify-center benben-flex-layout myTrack_flex_6_babdd"   v-if= " dataList.length<=0 && dataGoodsList.length>0" >
...
<!---订单列表flex布局结束-->
<!---flex布局flex布局开始-->
 <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout myTrack_flex_7_babdd"   v-if= " dataList.length<=0 && dataGoodsList.length>0" >
            <view   class='flex flex-wrap align-center justify-between myTrack_fd7_0_babdd'      >
            <template v-for = '(item,key0) in dataGoodsList'>
    <view   class='flex flex-direction flex-wrap align-stretch myTrack_fd7_0_c0_babdd'   @tap.stop="handleJumpDiy" data-type="navigateTo" :data-url="`/pages/fl/shopDetail/shopDetail?id=${item.aid}`"    :key='key0'>
...
<text class=' myTrack_fd7_0_c0_c3_c1_babdd'   >
    <text class=' myTrack_price1_fd7_0_c0_c3_c1_babdd' >{{ item.shop_price | frontPrice }}</text><text class=' myTrack_price2_fd7_0_c0_c3_c1_babdd' >{{ item.shop_price | laterPrice }}</text>
</text>
...
import {validate} from '@/common/utils/validate.js'
    ...
    export default {
        ...
        data(){
			return {"popupShow1720421246863":false,"popupShow1691652180194":false,"minixPagingListsApi":"","pageingListApiMethod":"","allowOnloadGetList":false,"dataList":[],"is_edit":false,/** shop_id【商品id】 **/ "shop_id":"","detele_id":"","true":true,"falses":false,"start_time":"","end_time":"","dataGoodsList":[],/** benbenPageloading【页面加载状态】 **/ "benbenPageloading":true};
		},
		computed:{
                  /**
     * @returns 
    ...
    */
    ...
    if(this.is_edit===true){
           this.is_edit = this.falses;
                ...
                this.pagingListPostDataContent = {goods_type:'2'}
                this.listData = [];
...
<style lang="scss" scoped>
        ...
        border-radius:16rpx 16rpx 16rpx 16rpx;background:#FFFFFF;width:344rpx;padding:0rpx 0rpx 24rpx 0rpx;box-shadow:0rpx 3rpx 30rpx #E8E8E8;margin:0rpx 0rpx 20rpx 0rpx;
        .myTrack_fd7_0_c0_c0_babdd{
        width:346rpx;height:346rpx;border-radius:16rpx 16rpx 0rpx 0rpx;
    }    .myTrack_fd7_0_c0_c1_babdd{
        color:#333333;font-size:28rpx;font-weight:700;line-height:40rpx;-webkit-line-clamp:2;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-box-orient:vertical;margin:24rpx 20rpx 0rpx 20rpx;
    }    .myTrack_fd7_0_c0_c2_babdd{
        margin:16rpx 20rpx 16rpx 20rpx;
        .myTrack_fd7_0_c0_c2_c0_babdd{
        color:#999999;font-size:24rpx;font-weight:400;line-height:33rpx;margin:0rpx 10rpx 0rpx 0rpx;
        ...
        color:#EF3A3A;font-weight:600;font-size:20rpx;
    }    .myTrack_fd7_0_c0_c3_c1_babdd{
        color:#ff5536;font-weight:700;font-size:20rpx;line-height:30rpx;margin:6rpx 0rpx 0rpx 0rpx;
        ...
        font-size:24rpx;color:var(--benbenFontColor2);text-decoration:line-through;margin:5rpx 0rpx 0rpx 10rpx;
    }    .myTrack_fd7_0_c0_c3_c2_c1_babdd{
        color:#B2B2B2;font-size:24rpx;font-weight:400;line-height:33rpx;text-decoration:line-through;margin:0rpx 0rpx -8rpx 0rpx;
    }}}}}
    .myTrack_flex_6_babdd{
        padding:51rpx 0rpx 33rpx 0rpx;
        .myTrack_fd6_1_babdd{
        font-size:40rpx;font-weight:600;color:#333;margin:0rpx 24rpx 0rpx 24rpx;
    }}
    .myTrack_fd6_0_babdd{
        width:138rpx;height:28rpx;
    }
    .myTrack_fd5_0_babdd{
        ...
        font-size:32rpx;background:var(--benbenbgColor1);color:var(--benbenFontColor5);width:270rpx;height:101rpx;line-height:101rpx;font-weight:500;
    }}}
    ...
    .myTrack_flex_2_babdd{
        ...
        .myTrack_fd2_0_babdd{
        width:400rpx;height:56rpx;border:1px solid rgba(191, 191, 191, 1);border-radius:8rpx;padding:0rpx 20rpx 0rpx 20rpx;
    ...
    .myTrack_fd2_0_c4_babdd{
        width:30rpx;height:30rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:4rpx 0rpx 0rpx auto;
    }
    .myTrack_fd2_1_c0_babdd{
        font-size:32rpx;font-weight:400;color:#333;
    }
    .myTrack_fd1_0_babdd{
        width:640rpx;background:#ffffff;border-radius:16rpx;padding:12rpx 012rpx 012rpx 012rpx;--active-bg-color:var(--benbenbgColor7);--range-bg-color:var(--benbenbgColor3);
    }
