<template>
  <view class="content" :style="{ top: top + StatusBarRpx + 'rpx' }">
    ...
    >
      ...
      <benben-empty :listData="lists"></benben-empty>
    </scroll-view>
    <!-- 右边列表结束 -->
  </view>
</template>
...
export default {
  ...
  computed: {
    toLeftId() {
      return `l_id${this.currentId >= 3 ? this.currentId - 3 : 0}`
    },
  },
  onLoad() {},
  mounted() {},
  methods: {
    ...
    handleSelect(index) {
      this.scrollTop = this.heightArr[index].minHeight + 2
      this.flag = true
      this.currentId = index
    },
    ...
    async getHeight() {
      ...
      res.map((item) => {
        let obj = {
          minHeight: height > 0 ? height : height,
        }
...
.cate-box-wrap {
  ...
  &:last-child {
    // min-height: calc(100vh - var(--benben-window-bottom, 0px) - var(--window-top));
    min-height: 100%;
  }
