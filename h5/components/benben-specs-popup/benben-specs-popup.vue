<template>
  <view>
    <benben-input-popup ref="inputPopup" z-index="1000" :min="minLimit" :max="maxNum" :step="1" @change="setnumber" />
    <benben-popup ref="spec" mode="bottom" z-index="100" v-model="show" :maskClick="true" class="popup">
        ...
        <view class="wrap" @tap.stop.prevent @touchmove.stop.prevent>
          ...
          <view class="margin-top">
            ...
            <view class="text-right text-theme text-sm">
              ...
              <text v-if="Number(minLimit) > 1">{{ $t('该商品') }}{{ minLimit }}{{ $t('件起售') }}</text>
            </view>
          </view>
          <view class="margin-top flex round overHidden">
            <view v-if="['cart', 'all'].includes(type)" class="flex-sub btn color1" @tap.stop="handleSubmit('addCart')">
              {{ $t(addName) }}
            </view>
            <view v-if="['buy', 'all'].includes(type)" class="flex-sub btn color2" @tap.stop="handleSubmit('buyNow')">{{ $t(buyName) }}</view>
...
export default {
  ...
  methods: {
    ...
    handleInput() {
      if (this.disabled || this.inhibitInput) return
