<template>
  <view class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
    ...
    <view class="fixed-bottom safe-area-inset-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
      ...
      <view v-if="!choosable" class="choose-btn" @click="cropClick">{{ $t('确定') }}</view>
      <block v-else-if="!!imgSrc">
        <view class="rechoose" @click="chooseImage">{{ $t('重选') }}</view>
        <button class="button" size="mini" @click="cropClick">{{ $t('确定') }}</button>
      </block>
      <view v-else class="choose-btn" @click="chooseImage">{{ $t('选择图片') }}</view>
    </view>
  </view>
</template>
