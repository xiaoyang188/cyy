    ...
    <view class="body">
      ...
      <view class="day-list">
        <view class="day-item" v-for="(day, index) in dayList" :key="index" @click="handleClick(day)">
          <text v-if="day.checked" class="checked" :class="{ clickStyle: day.isclickActive }">{{ day.day }}</text>
          <text v-else-if="day.isToday" class="today" :class="{ clickStyle: day.isclickActive }">{{ day.day }}</text>
          <text v-else-if="day.type === 'current'" class="current-month-day" :class="{ clickStyle: day.isclickActive }">{{ day.day }}</text>
          <text v-else :class="{ clickStyle: day.isclickActive }">{{ day.day }}</text>
        </view>
      </view>
    </view>
  </view>
  ...
  methods: {
    ...
    handleClick(day) {
      for (var i = 0; i < this.dayList.length; i++) {
        this.dayList[i].isclickActive = false
      }
      day.isclickActive = true
      // 构造数据
      const payload = {
        year: this.year,
        month: this.month,
      ...
      switch (day.type) {
        ...
        default:
          this.emitClickDay({
            ...
            day: payload.day,
            date: payload.year + '-' + this.formatNumber(payload.month) + '-' + this.formatNumber(payload.day),
          })
      }
    },
    // 触发选中日期事件
    emitClickDay(payload) {
      this.timer && clearTimeout(this.timer)
      this.timer = setTimeout(() => {
        this.$emit('clickDay', payload)
      }, 1000)
    },
    formatNumber(str) {
  ...
  .body {
    .weeks {
      ...
      .week-item {
        flex: 1;
        text-align: center;
        background: var(--week-bg-color, #fff);
        color: var(--week-text-color, #333);
      }
    }
    ...
    .day-list {
      ...
      .day-item {
        ...
        padding: 15rpx 0;
        font-size: 28rpx;
        color: var(--lr-text-color, #c8c8c8);
        line-height: 56rpx;

        .current-month-day {
          color: var(--calendar-text-color, #171717);
        }

        .today {
          width: 56rpx;
          border-radius: 50%;
          background: var(--today-bg-color, #fc9c3a);
          color: var(--today-text-color, #fff);
        }

        .checked {
          border-radius: 50%;
          color: var(--checked-text-color, #171717);
          background: var(--checked-bg-color, transparent);
          ...
          &::before {
            ...
            height: 10rpx;
            border-radius: 50%;
            background: var(--checked-bg-color, #f4965f);
          }
        }
        ...
        .clickStyle {
          width: 56rpx;
          border-radius: 50%;
          background: var(--click-bg-color, #eb681e);
          color: var(--click-text-color, #fff);
        }
      }
    }
  }
