    ...
    <view class="page">
      <view
        ...
        <view class="flex flex-wrap align-stretch justify-between myInfo_fd0_0_babdd">
          <view class="flex flex-wrap align-center myInfo_fd0_0_c0_babdd" @tap.stop="handleJumpDiy" data-type="back"
            data-url="1">
        ...
        <view class="myInfo_card_babdd">
          <view class="myInfo_cell_babdd myInfo_cell_avatar_babdd">
            ...
            <view class="myInfo_cell_body_babdd">
              <benben-image-upload :path.sync="dataDetails.avatar" :img-id.sync="dataDetails.avatar" :is-cropping="true"
                :is-show-clean="false" class="upload position-relative myInfo_avatar_upload_babdd">
                <image class="myInfo_avatar_img_babdd" mode="aspectFill"
                  :src="dataDetails.avatar ? dataDetails.avatar : STATIC_URL + '331.png'"></image>
              </benben-image-upload>
            </view>
...
<text class='fu-iconfont2  myInfo_fd1_0_c0_c2_babdd'  >&#xe792;</text>
 ...
 -->
          <view class="myInfo_cell_babdd" @tap="handleJumpDiy" data-type="navigateTo"
            :data-url="`/pages/wd/editNameX/editNameX`">
            <text class="myInfo_cell_label_babdd">{{ $t('昵称') }}</text>
            <view class="myInfo_cell_body_babdd">
              <text class="myInfo_cell_value_babdd"
          ...
          <view class="myInfo_cell_babdd">
            ...
            <view class="myInfo_cell_body_babdd">
              <text class="myInfo_cell_value_babdd"
                :class="{ myInfo_cell_value_placeholder_babdd: !dataDetails._mobile }">
          ...
          <view class="myInfo_cell_babdd" @tap.stop="pickerDiy1656985489961 = true">
            ...
            <view class="myInfo_cell_body_babdd">
              <text class="myInfo_cell_value_babdd"
                :class="{ myInfo_cell_value_placeholder_babdd: !sex }">{{ sex || $t('请选择性别') }}</text>
            </view>
            <text class="fu-iconfont2 myInfo_cell_arrow_babdd">&#xe792;</text>
          </view>
      ...
      <!---我的资料flex布局结束-->
      <!--选择生日开始 -->
      <benben-picker class="" ref="benbenWritePickerCodepicker2" :visible.sync="pickerDiy1656986153485"
        ...
        <template #picker-header>
          <view class="flex flex-wrap align-center justify-between myInfo_picker2_0_babdd">
            <text class="myInfo_picker2_0_c0_babdd"
              @tap="$refs.benbenWritePickerCodepicker2.cancel()">{{ $t('取消') }}</text>
            <text class="myInfo_picker2_0_c1_babdd">{{ $t('选择日期') }}</text>
            <text class="myInfo_picker2_0_c2_babdd"
              @tap="$refs.benbenWritePickerCodepicker2.pickerConfirm()">{{ $t('确定') }}</text>
          </view>
        </template>
      </benben-picker>
      <!--选择生日结束 -->
      <!--选择性别开始 -->
      <benben-picker class="" ref="benbenWritePickerCodepicker3" :visible.sync="pickerDiy1656985489961"
        :label.sync="sex" :value.sync="dataDetails.gender" :options="sexType" mode="selector" :mask-show="true"
        ...
        <template #picker-header>
          <view class="flex flex-wrap align-center justify-between myInfo_picker3_0_babdd">
            <text class="myInfo_picker3_0_c0_babdd"
              @tap="$refs.benbenWritePickerCodepicker3.cancel()">{{ $t('取消') }}</text>
            <text class="myInfo_picker3_0_c1_babdd">{{ $t('选择性别') }}</text>
            <text class="myInfo_picker3_0_c2_babdd"
              @tap="$refs.benbenWritePickerCodepicker3.pickerConfirm()">{{ $t('确定') }}</text>
          </view>
        </template>
      </benben-picker>
      <!--选择性别结束 -->
