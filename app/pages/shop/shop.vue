<template>
  <view class="container">
    ...
    <view class="shop" style="width: 100%; margin-top: 30rpx">
      <button @click="toWebView">跳转</button>
...
function buildShopWebPath(token, info) {
  const base = `${SHOP_H5_BASE}__app_webview=1&user_token=${encodeURIComponent(token)}`;
  ...
  return `${base}&safe_area_top=${safeTop}&status_bar_height=${statusBar}&safe_area_bottom=${safeBottom}`;
}
