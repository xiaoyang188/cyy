...
export default {
  ...
  created() {
    this.sections = [this.$t('上午'), this.$t('下午')]
  ...
  methods: {
    ...
    setStartEnd() {
      ...
      // 设置最大最小时
      this.startObj.sections = sh > 12 ? [this.$t('下午')] : [this.$t('上午'), this.$t('下午')]
      this.endObj.sections = eh > 12 ? [this.$t('上午'), this.$t('下午')] : [this.$t('上午')]
    },
    // 初始化选中项
    ...
    trimDate(val, month, day) {
      ...
      if (!sections) {
        sections = curr.getHours() > 12 ? this.$t('下午') : this.$t('上午')
      }
      return {
        val: curr,
        y: curr.getFullYear(),
