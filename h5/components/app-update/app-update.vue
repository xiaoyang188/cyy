<template>
  <benben-popup v-model="isShow" mode="center" border-radius="16" :mask-close-able="false">
    <view class="update-box">
      <view class="title">{{ $t('发现新版本') }}</view>
      <scroll-view scroll-y="true" class="content">
        <jyf-parser :html="content | richTextFormat"></jyf-parser>
      </scroll-view>
      <view class="progress-bar" v-if="loading > 0">
        <view class="progress-center" :style="[{ width: loading + '%' }]"></view>
      </view>
          ...
          if (downloadResult.statusCode === 200) {
            plus.runtime.install(
              ...
              (error) => {
                ...
                this.$message.info(global.i18n.t('安装失败'))
              }
            )
          }
        },
