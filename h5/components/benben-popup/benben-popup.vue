<template>
  <view
    ...
    :class="{ 'benben-drawer-visible': showDrawer }"
    class="benben-drawer"
  ...
  >
    ...
    ></fu-mask>
    <view
      class="benben-drawer-content"
      @tap="modeCenterClose(mode)"
      :class="['benben-drawer-' + mode, showDrawer ? 'benben-drawer-content-visible' : '', zoom && mode == 'center' ? 'benben-animation-zoom' : '']"
      :style="[style]"
    >
      <view class="benben-mode-center-box" v-if="mode == 'center'" :style="[centerStyle]">
        <scroll-view
          @tap.stop.prevent="touchmoveStop"
          @touchmove.stop.prevent="touchmoveStop"
          style="max-height: 80vh"
          class="benben-drawer-scroll-view"
          scroll-y
        >
          <slot />
        </scroll-view>
      ...
      <view v-if="safeAreaInsetBottom" class="benben-safe-area-bottom"></view>
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    ...
    style() {
      ...
      return { ...style, background: this.backgroundColor }
    },
    // 中部弹窗的特有样式
    centerStyle() {
      let style = {}
  ...
  watch: {
    value(val) {
      if (val == this.visibleSync) return
      if (val) {
        this.open()
      } else {
        this.close()
...
.benben {
  ...
  &-drawer-content-visible.benben-drawer-center {
    transform: scale(1);
    opacity: 1;
  }
