<template>
    <page-body   >
        <view class="page" >
    <view class="flex align-stretch benben-position-layout flex search_flex_0_babdd"    :style="{height:(100+StatusBarRpx)+'rpx',paddingTop:StatusBarRpx+'rpx', }">
...
<benben-input class='flex-sub search_fd0_0_c1_c1_babdd'   type="text" :focus='true' :placeholder="`${name ? name : '请输入想要搜索的商品'}`" confirm-type="done" :maxlength="-1" placeholder-style="color:var(--benbenFontColor1);font-size:24rpx"  @confirm="addSearchHistoryFunc()" @input="searchFunc()" v-model="keyword"  />
<image class='search_fd0_0_c1_c2_babdd'  mode="aspectFit"  :src='STATIC_URL+"779.png"' @tap="ClearInputFunc()" v-if= " keyword!=''"></image>
</view>
    <view   class='flex flex-wrap align-center search_fd0_0_c2_babdd'    v-if= " appSystemIdentification=='Wechat'"  >
        </view>
</view>
...
<image class='search_fd1_0_c0_c2_babdd'  mode="aspectFit"  :src='STATIC_URL+"386.png"' @tap.stop="popupShow1661481796822=true" v-if= " appSearchHistory.length>0"></image>
</view>
    ...
    <view   class='flex align-center search_fd1_0_c1_c0_babdd'   @tap.stop="handleJumpDiy" data-type="navigateTo" :data-url="`/pages/fl/commodityList/commodityList?keyword=${item.name}`" @longpress="handleLongpressFunc(item.name)"    :key='key0'>
        <text class='search_fd1_0_c1_c0_c0_babdd'  >{{item.name}}</text>
<image class='search_fd1_0_c1_c0_c1_babdd'  mode="aspectFit"  :src='STATIC_URL+"387.png"' @tap.stop="deleteSearchHistoryFunc(item.name)" v-if= " item.name==removeIndex"></image>
</view>
    </template>
...
<text class='search_fd2_1_babdd'  >{{$t('确定删除历史记录？')}}</text>
...
<button class='search_fd2_2_c1_babdd'  @tap.stop="emptySearchHistoryFunc()"  >{{$t('确定')}}</button>
</view>
</view>
...
<text class='search_fd3_0_c2_babdd'     @tap.stop="changeSweetGrilFunc()" >{{$t('换一批')}}</text>
    ...
    <view   class='flex flex-wrap align-center search_fd3_1_c0_babdd'   @tap.stop="clickDiscoverFunc(item.name)" v-if= "item.is_set=='1'"   :key='key0'>
...
<image class='search_fd3_1_c0_c1_babdd'  mode="aspectFit"  :src='STATIC_URL+"389.png"'></image>
    ...
    <template v-for = '(item,key0) in dataList'>
    <view   class='flex flex-wrap align-center search_fd3_1_c1_babdd'   @tap.stop="clickDiscoverFunc(item.name)" v-if= "item.is_set=='0'"   :key='key0'>
...
<!---flex布局flex布局结束-->
 ...
 <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout search_flex_4_babdd"   v-if= " associateList.length==0 || keyword==''" >
...
<text class='search_fd4_0_c1_babdd'  >{{$t('全网热搜榜')}}</text>
    ...
    <view   class='flex flex-wrap align-center search_fd4_1_c0_babdd'   @tap.stop="handleJumpDiy" data-type="navigateTo" :data-url="`/pages/fl/shopDetail/shopDetail?id=${item.aid}`"    :key='key0'>
        <image class='search_fd4_1_c0_c0_babdd'  mode="aspectFit"  :src='STATIC_URL+"391.png"'  v-if= "key0=='0'"></image><image class='search_fd4_1_c0_c0_babdd'  mode="aspectFit"  :src='STATIC_URL+"393.png"'  v-if= "key0=='1'"></image><image class='search_fd4_1_c0_c0_babdd'  mode="aspectFit"  :src='STATIC_URL+"395.png"'  v-if= "key0=='2'"></image>
...
<text class='flex-sub search_fd4_1_c0_c2_babdd'      >{{item.name}}</text>
    ...
    <view   class='flex flex-wrap align-center'       :key='key1'>
        <image class='search_fd4_1_c0_c3_c0_babdd' :style="{'opacity':`${showIcon}`}" mode="aspectFit"  :src='STATIC_URL+"394.png"'  v-if= " in_array(child,'1')"></image>
<image class='search_fd4_1_c0_c3_c0_babdd' :style="{'opacity':`${showIcon}`}" mode="aspectFit"  :src='STATIC_URL+"392.png"'  v-if= " in_array(child,'2')"></image>
</view>
    </template>
    ...
    <view   class='flex align-center justify-between'   @tap.stop="handleJumpDiy" data-type="navigateTo" :data-url="`/pages/sy/searchList/searchList`"   >
            <view   class='flex flex-wrap align-center justify-center search_fd4_1_c1_c0_babdd'   @tap.stop="handleJumpDiy" data-type="navigateTo" :data-url="`/pages/sy/searchList/searchList`"   >
...
<!---flex布局flex布局开始-->
 <view class="flex flex-direction align-stretch benben-flex-layout search_flex_5_babdd"   v-if= " associateList.length!=0 && keyword!=''" >
    ...
    <view   class='flex align-center justify-between search_fd5_0_babdd'   @tap="goCommodityListFunc(key0)"    :key='key0'>
                <jyf-parser class='search_fd5_0_c0_babdd'  :html="item.name | richTextFormat"  >
        </jyf-parser>
...
<!---flex布局flex布局结束-->
    <view class="flex benben-position-layout flex flex-wrap align-center search_flex_6_babdd"   @tap.stop="popupShow1730704961807=true" :style="{'bottom':`${keyboardheight}px`}">
...
<benben-popup v-model="popupShow1730704961807" :mask="true" :mask-close-able="true"  mode='bottom' :z-index='999' >
 ...
 <view class="flex flex-direction flex-wrap align-stretch benben-flex-layout search_flex_7_babdd"    >
            ...
            <view   class='flex flex-wrap align-center search_fd7_0_c0_babdd'      >
        </view>
    ...
    <view   class='flex flex-wrap align-center justify-end search_fd7_0_c2_babdd'      >
        <image class='search_fd7_0_c2_c0_babdd'  mode="aspectFit"  :src='STATIC_URL+"85.png"' @tap.stop="popupShow1730704961807=false" ></image>
</view>
</view>
    <view   class='flex flex-direction align-center search_fd7_1_babdd'      >
        <text class='search_fd7_1_c0_babdd'    v-if= " is_recording===false"  >{{$t('长按说话，松手识别')}}</text><text class='search_fd7_1_c0_babdd'    v-if= " is_recording===true"  >{{$t('请说,我在聆听...')}}</text>
...
<script>
    ...
    export default {
        ...
        data(){
			return {"popupShow1730704961807":false,"popupShow1661481796822":false,"keyword":"",/** dataList【搜索发现】 **/ "dataList":[],"dataLists":[],"removeIndex":-1,"xainshi":false,"associateList":[],"inputList":[],"name":"",/** Timer【定时器】 **/ "Timer":"",/** showIcon【闪透明度】 **/ "showIcon":"1","flag":true,"keywordList":[],"language":[{"name":"中文","value":"1","image":""},{"name":"English","value":"2","image":""},{"name":"English13333333","value":"3","image":""}],"language_value":"1","language_label":"中文","is_recording":false,"keyboardheight":"40"};
		},
		computed:{
              ...
              this.xainshi = true;
                ...
                //点击发现
                 clickDiscoverFunc(name){
                                   this.$store.commit('appSearchHistoryAdd',{"name":name})
...
<style lang="scss" scoped>
        ...
        .search_fd7_0_babdd{
        border-radius:16rpx 16rpx 0rpx 0rpx;background:var(--benbenbgColor1);width:750rpx;padding:32rpx 32rpx 32rpx 32rpx;
        .search_fd7_0_c0_babdd{
        width:100rpx;height:50rpx;
    }    .search_fd7_0_c1_c0_babdd{
        margin:0rpx 32rpx 0rpx 32rpx;color:var(--benbenFontColor0);font-size:32rpx;font-weight:500;
    }    .search_fd7_0_c2_babdd{
        width:100rpx;
        .search_fd7_0_c2_c0_babdd{
        width:40rpx;height:40rpx;border-radius:0rpx;
    }}}    .search_fd7_1_babdd{
        margin:40rpx 0rpx 80rpx 0rpx;
        .search_fd7_1_c0_babdd{
        margin:0rpx 0rpx 24rpx 0rpx;color:var(--benbenFontColor0);
    }    .search_fd7_1_c1_babdd{
        width:200rpx;height:200rpx;border-radius:0rpx;
    }}    .search_fd7_2_c0_c0_babdd{
        margin:32rpx 0rpx 32rpx 0rpx;color:rgba(221, 221, 221, 1);font-size:32rpx;font-weight:500;
    }    .search_fd7_2_c1_c0_babdd{
        margin:32rpx 0rpx 32rpx 0rpx;color:rgba(174, 174, 174, 1);font-size:28rpx;font-weight:500;
    }}
    .search_flex_6_babdd{
        overflow:hidden;z-index:10;bottom:calc(80rpx + var(--benben-window-bottom,0px));left:43%;box-shadow:0rpx 3rpx 30rpx #E8E8E8;border-radius:50rpx;background:var(--benbenbgColor1);background-size:100% auto;
        .search_fd6_0_babdd{
        width:66rpx;height:66rpx;border-radius:0rpx;margin:0rpx 24rpx 0rpx 24rpx;
    }}
    .search_flex_5_babdd{
        padding:24rpx 24rpx 0rpx 24rpx;
        .search_fd5_0_babdd{
        padding:32rpx 0rpx 32rpx 0rpx;border-bottom:1px solid #eee;
        .search_fd5_0_c1_babdd{
        width:20rpx;height:20rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:0rpx 0rpx 0rpx 24rpx;
    }}}
    .search_flex_4_babdd{
        ...
        width:34rpx;height:34rpx;margin:0rpx 16rpx 0rpx 0rpx;
    }    .search_fd4_0_c1_babdd{
        font-size:32rpx;font-weight:700;color:var(--benbenFontColor0);
    }}    .search_fd4_1_babdd{
        border-radius:24rpx 24rpx 24rpx 24rpx;box-shadow:0rpx 0rpx 16rpx rgba(0,0,0,0.0800);background:var(--benbenbtnColor1);background-size:100% auto !important;padding:32rpx 24rpx 56rpx 24rpx;
        .search_fd4_1_c0_babdd{
        margin:0rpx 0rpx 40rpx 0rpx;
        .search_fd4_1_c0_c1_babdd{
        font-size:28rpx;font-weight:400;color:var(--benbenFontColor0);margin:0rpx 34rpx 0rpx 11rpx;
    }    .search_fd4_1_c0_c2_babdd{
        margin:0rpx 16rpx 0rpx 0rpx;font-size:28rpx;font-weight:400;color:var(--benbenFontColor0);-webkit-line-clamp:1;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-box-orient:vertical;
    }}    .search_fd4_1_c1_c0_babdd{
        margin:16rpx 0rpx 0rpx 0rpx;border-radius:16rpx 16rpx 16rpx 16rpx;background:var(--benbenbgColor1);width:638rpx;height:80rpx;border:1px solid var(--benbenbdColor2);
        .search_fd4_1_c1_c0_c0_babdd{
        width:32rpx;height:32rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:0rpx 8rpx 0rpx 0rpx;
    }    .search_fd4_1_c1_c0_c1_babdd{
        font-size:24rpx;font-weight:400;color:var(--benbenFontColor4);
    }}}}
    .search_fd4_1_c0_c0_babdd{
        width:38rpx;height:38rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:0rpx 24rpx 0rpx 0rpx;
    }
    .search_fd4_1_c0_c3_c0_babdd{
        width:40rpx;height:40rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:0rpx 24rpx 0rpx 0rpx;transition:all ease 0.2s;
    }
    .search_fd3_0_babdd{
        padding:32rpx 0rpx 15rpx 0rpx;margin:0rpx 24rpx 0rpx 24rpx;
        .search_fd3_0_c0_babdd{
        width:34rpx;height:34rpx;border-radius:17rpx 17rpx 17rpx 17rpx;margin:0rpx 16rpx 0rpx 0rpx;
    }    .search_fd3_0_c1_babdd{
        font-size:32rpx;font-weight:700;color:var(--benbenFontColor0);
    }    .search_fd3_0_c2_babdd{
        margin:0rpx 0rpx 0rpx auto;padding:8rpx 16rpx 8rpx 16rpx;border-radius:8rpx 8rpx 8rpx 8rpx;border:1px solid var(--benbenbdColor0);color:var(--benbenFontColor0);font-size:24rpx;font-weight:400;line-height:33rpx;
    }}
    .search_fd3_1_babdd{
        padding:0rpx 0rpx 0rpx 12rpx;
        .search_fd3_1_c0_babdd{
        border-radius:32rpx 32rpx 32rpx 32rpx;background:var(--benbenbtnColor1);background-size:100% auto !important;padding:0rpx 40rpx 0rpx 40rpx;line-height:56rpx;margin:0rpx 12rpx 24rpx 12rpx;border:1px solid var(--benbenbdColor2);position:relative;
        .search_fd3_1_c0_c0_babdd{
        font-size:24rpx;font-weight:400;color:var(--benbenFontColor4);
    }    .search_fd3_1_c0_c1_babdd{
        width:18rpx;height:32rpx;border-radius:0rpx 0rpx 0rpx 0rpx;position:absolute;top:-8rpx;right:4rpx;
    }}    .search_fd3_1_c1_babdd{
        border-radius:32rpx 32rpx 32rpx 32rpx;background:var(--benbenbtnColor3);background-size:100% auto !important;padding:0rpx 40rpx 0rpx 40rpx;line-height:56rpx;margin:0rpx 12rpx 24rpx 12rpx;
        .search_fd3_1_c1_c0_babdd{
        font-size:24rpx;font-weight:400;color:var(--benbenFontColor0);
    }}}
    .search_flex_2_babdd{
        margin:0rpx 105rpx 0rpx 105rpx;background:#fff;background-size:#eee;border-radius:25rpx 25rpx 25rpx 25rpx;padding:40rpx 0rpx 0rpx 0rpx;
        .search_fd2_0_babdd{
        ...
        line-height:110rpx;border-radius:0rpx 0rpx 0rpx 16rpx;font-size:32rpx;background:var(--benbenbtnColor1);color:var(--benbenFontColor2);border-right:1px solid var(--benbenbdColor0);height:110rpx;width:270rpx;
    }    .search_fd2_2_c1_babdd{
        line-height:110rpx;border-radius:0rpx 0rpx 16rpx 0rpx;font-size:32rpx;background:var(--benbenbtnColor1);color:var(--benbenFontColor0);width:269rpx;height:110rpx;
    }}}
    .search_fd1_0_c0_babdd{
        padding:32rpx 0rpx 24rpx 0rpx;margin:0rpx 24rpx 0rpx 24rpx;
        ...
        width:24rpx;height:28rpx;border-radius:0rpx 0rpx 0rpx 0rpx;margin:0rpx 0rpx 0rpx auto;
    }}
    .search_fd1_0_c1_babdd{
        ...
        font-size:24rpx;font-weight:400;color:var(--benbenFontColor0);margin:0rpx 16rpx 0rpx 0rpx;-webkit-line-clamp:1;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-box-orient:vertical;max-width:200rpx;
    }    .search_fd1_0_c1_c0_c1_babdd{
        width:18rpx;height:18rpx;border-radius:0rpx 0rpx 0rpx 0rpx;
    }}}
    .search_fd1_0_c2_babdd{
        font-size:24rpx;font-weight:400;color:var(--benbenFontColor1);margin:0rpx 0rpx 32rpx 24rpx;
    }
    .search_flex_0_babdd{
        width:750rpx;height:100rpx;z-index:10;top:0rpx;background:#fff;background-size:100% auto !important;
        .search_fd0_0_babdd{
        margin:17rpx 32rpx 24rpx 32rpx;
        .search_fd0_0_c0_babdd{
        font-size:36rpx;color:var(--benbeniconColor1);font-weight:500;
    }    .search_fd0_0_c1_babdd{
        background:var(--benbenbgColor0);background-size:100% auto !important;border-radius:34rpx 34rpx 34rpx 34rpx;margin:0rpx 0rpx 0rpx 24rpx;padding:0rpx 24rpx 0rpx 24rpx;height:64rpx;
        .search_fd0_0_c1_c0_babdd{
        width:24rpx;height:24rpx;border-radius:0rpx 0rpx 0rpx 0rpx;
    }    .search_fd0_0_c1_c1_babdd{
        font-size:24rpx;color:var(--benbenFontColor0);margin:0rpx 0rpx 0rpx 12rpx;
    ...
    ::v-deep .search_fd7_2_babdd{
        padding:16rpx 32rpx 48rpx 32rpx;margin:0rpx 0rpx 0rpx 0rpx;
    }
