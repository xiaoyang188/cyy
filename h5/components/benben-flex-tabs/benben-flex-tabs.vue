<template>
  <view :class="classStr" :style="headStyle" :id="`${selectMark}-box`">
    <view class="benben-flex-tabs" :class="classStr" :style="[stickyContent, styleInfo]">
      <slot></slot>
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    ...
    headStyle() {
      let style = `box-sizing: content-box;padding-top:${this.openTitleType ? this.StatusBarRpx : 0}rpx;`
      Object.keys(this.styleInfo).forEach((key) => {
        style += `${key}:${this.styleInfo[key]};`
      })
      return style
    },
    stickyContent() {
        ...
        uni
          ...
          .exec(function (rects) {
            ...
            query.exec(function (res) {
              ...
              that.$emit('update:tabs-info', {
                ...that.tabsInfo,
                lineleft: itemLeft + 'px',
                lineWidth: lineWidth + 'px',
                moveX: moveX,
              })
              if (!that.tabsInfo.isInit) {
                setTimeout(() => {
                  that.$emit('update:tabs-info', {
                    ...that.tabsInfo,
                    isInit: true,
