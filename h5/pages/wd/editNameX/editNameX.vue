<template>
  <page-body>
    <view class="page">
      <view class="flex benben-position-layout flex flex-wrap align-center editNameX_flex_0_babdd"
        :style="{ height: 88 + StatusBarRpx + 'rpx', paddingTop: StatusBarRpx + 'rpx' }">
        <view class="flex flex-wrap align-center justify-between flex-sub editNameX_fd0_0_babdd">
          <view class="flex flex-wrap align-center editNameX_fd0_0_c0_babdd" @tap.stop="handleJumpDiy" data-type="back"
            data-url="1">
          ...
          <view class="flex flex-wrap align-center justify-end editNameX_fd0_0_c0_babdd">
            <button class="editNameX_fd0_0_c2_c0_babdd" @tap.stop="editUserNiknameFunc()"
      ...
      <view class="flex flex-direction align-stretch benben-flex-layout editNameX_flex_1_babdd">
        <view class="flex flex-wrap align-center editNameX_fd1_0_babdd">
          <benben-input class="flex-sub editNameX_fd1_0_c0_babdd" type="text" :placeholder="$t('请输入昵称')"
            confirm-type="done" :maxlength="13" placeholder-style="color:var(--benbenFontColor2);font-size:36rpx"
            @input="valueInspectFunc($event)" v-model="userInfo.nickname" />
        </view>
        <view class="flex flex-wrap align-center editNameX_fd1_1_babdd">
          <text class="editNameX_fd1_1_c0_babdd">{{ $t('请设置2-13个字符，不包含@《/等无效字符哦') }}</text>
        </view>
        <view class="flex align-center justify-center editNameX_fd1_2_babdd">
          <button class="editNameX_fd1_2_c0_babdd" @tap.stop="editUserNiknameFunc()"
            v-if="appSystemIdentification == 'Wechat'">
            {{ $t('保存') }}
          </button>
        </view>
  ...
  export default {
    ...
    methods: {
      ...
      async editUserNiknameFunc() {
        ...
        if (this.userInfo.nickname.length > 13) {
